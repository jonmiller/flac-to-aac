...
import com.tetyl.flactoaac.exceptions.ExecutableProcessError;
...
import com.tetyl.flactoaac.utilities.ProcessStreamRedirector;

public class TrackFile
{
	private static Logger logger = Logger.getLogger(TrackFile.class.getName());
	
	private static final String metaflacLocation = 
		"C:" + File.separator + "Program Files (x86)" + File.separator + "Flac" + File.separator + "metaflac.exe";

	private static final String flacLocation = 
		"C:" + File.separator + "Program Files (x86)" + File.separator + "Flac" + File.separator + "flac.exe";

	private static final String neroEncoderLocation = 
		"C:" + File.separator + "Program Files (x86)" + File.separator + "NeroAAC" + File.separator + "win32" + File.separator + "neroAacEnc.exe";
	
	private static final String atomicParsleyLocation = 
		"C:" + File.separator + "Program Files (x86)" + File.separator + "Atomic Parsley" + File.separator + "AtomicParsley.exe";
	...
	void populateMetadata(boolean deleteMetadataFileWhenDone, int numTracks) throws NonExistenceException, MetaDataFileException
	{
		if(exists)
		{
			...
			String[] commandArray = 
					new String[] {metaflacLocation, "--no-utf8-convert", "--export-tags-to=" + metadataPath, file.getAbsolutePath()};
			...
			try
			{
				...
				if(returnStatus != 0)
				{
					throw new ExecutableProcessError("Metaflac returned with an error status code " +
							"when called with:"  + concatenateCommandArray(commandArray));
				}
			}
			...
			try
			{
				BufferedReader metadataReader = 
					new BufferedReader(new InputStreamReader(new FileInputStream(metadataFile), Charset.forName("UTF-8")));
		...
		{
			throw new NonExistenceException("Attempted to call populateMetadata " +
			"on a TrackFile that does not exist");
		}
	}

	void convertTo(TrackFile destinationTrackFile) 
	throws NonExistenceException, WrongTypeException
	{
		if(exists)
		{
			if(destinationTrackFile.getType().equals(Type.WAV))
			{
				...
				{
					throw new WrongTypeException("Attempted to convert something other " +
					"than a FLAC file to a WAV file");
				}
			...
			else if(destinationTrackFile.getType().equals(Type.AAC))
			{
				...
				{
					throw new WrongTypeException("Attempted to convert something other " +
					"than a WAV file to an AAC file");
				}
			}
		}
		else
		{
			throw new NonExistenceException("Attempted to call convert " +
			"on a TrackFile that does not exist");
		}
	}
	...
	void writeMetadataToFile() throws WrongTypeException, NonExistenceException
	{
		...
		{
			throw new NonExistenceException("Attempted to call writeMetadataToFile " +
			"on a TrackFile that does not exist");
		}
	}

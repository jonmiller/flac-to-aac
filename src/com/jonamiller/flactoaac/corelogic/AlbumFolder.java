...
public class AlbumFolder {
  ...
  void populateChildren() throws NonExistenceException, MetaDataFileException {
    if (exists) {
      ...
      throw new NonExistenceException("Attempted to call populateChildren " + "on an AlbumFolder " +
          "that does not exist");
    }
  }

  void copyStructureTo(
      AlbumFolder destinationAlbumFolder) throws NonExistenceException, MetaDataFileException,
      WrongTypeException {
    if (exists) {
      for (TrackFile t : trackFiles) {
      ...
      throw new NonExistenceException("Attempted to call copyStructureTo " + "on a AlbumFolder " +
          "that does not exist");
    }
  }
